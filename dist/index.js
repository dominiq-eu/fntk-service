...
const program = fn => {
    ...
    return Http.createServer(prog).listen(port, () => log.debug(`Listening on: 0.0.0.0:${port}`));
};
...
const TelegramGateway = function ({ token, parseMode = ParseMode.Text() }) {
    ...
    return fn => {
        ...
        bot.on('text', msg => {
            ...
            return handle(req).then(response => {
...
const cleanStopwords = (text, lang) => text
...
const getMatch = (stemmer, text, sentences) => sentences.map(s => calcSimilarity(stemmer, text, s)).sort(([val_a, s_a], [val_b, s_b]) => val_b - val_a)
// .map(log.debug('#'))
...
const getMatches = functions => line => functions.reduce((ret, fn) => ret.concat([{
    ...
    path: fn.path
}]), []).filter(fn => fn.propability > 0).sort((a, b) => b.propability - a.propability);
